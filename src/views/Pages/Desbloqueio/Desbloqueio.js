...
class Desbloqueio extends Component {

  constructor(props) {
    super(props);

    let valores = atob(this.props.match.params.idDesbloqueio);
    let dados = valores.split(":");
    let nome = atob(dados[1]);

    this.state = {
      id_desbloqueio : dados[0],
    ...
    if(localStorage.getItem('id_desbloqueio') !== this.state.id_desbloqueio) {
      localStorage.clear();
    }
  }
  ...
  autorizaDesbloqueio = async () => {
    const url = "https://app.factafinanceira.com.br/DesbloqueioBeneficio/ApiDesbloqueioBeneficio";
    const FormData = require('form-data');

    const formData = new FormData();
    formData.append('id_desbloqueio', this.state.id_desbloqueio);
    ...
    await axios.post(url, formData, {
      ...
      localStorage.setItem('id_desbloqueio', this.state.id_desbloqueio);
    })
    .catch((error) => {
      console.log(error);
    })
  ...
  componentDidMount () {
    if (localStorage.getItem('id_desbloqueio')) {
       this.setState({confirmado : true});
    }
  }
                    ...
                    { this.state.confirmado === false
                      ?
                       <>
                            <Row>
                              <Col>
                                <h4 className="text-center mb-3 font-weight-bold">Termo de Autorização de desbloqueio de benefício</h4>
                                <Row className="mt-5">
                                  <Col className="text-justify" md="12" lg="12" xs="12" sm="12">
                                    <p>
                                      Eu <span className="font-weight-bold">{ this.state.nome }</span>, CPF <span className="font-weight-bold">{ this.state.cpf }</span>, autorizo o INSS/DATAPREV a desbloquear o benefício <span className="font-weight-bold">{ this.state.beneficio }</span>, para que seja possível realizar a contratação de empréstimo consignado ou cartão consignado de benefícios do INSS.
                                    </p>
                                    <Row className="mt-3">
                          ...
                          <Col>
                            ...
                            <Row className="mt-5">
                              <Col className="text-center" md="12" lg="12" xs="12" sm="12">
                                <p>
                                  Sr(a).  <span className="font-weight-bold">{ this.state.nome }</span> recebemos o seu pedido de desbloqueio de benefício com sucesso!
                                </p>
                              </Col>
                            </Row>
                          </Col>
